...
export default function StockOutModal({ items = [], show, onClose }) {
    ...
    const selectedItem = items.find(
        (item) => item.id === parseInt(data.item_id)
    );
    const newStock =
    ...
    const insufficientStock =
        selectedItem &&
        data.quantity &&
        parseInt(data.quantity) > parseInt(selectedItem.current_stock);
                    ...
                    <form onSubmit={handleSubmit}>
                        <div className="bg-white p-4 sm:p-6 space-y-4">
                            ...
                            <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                                <p className="text-xs text-blue-800">
                                    <span className="font-medium">Note:</span> A
                                    unique transaction code will be
                                    automatically generated for this stock
                                    movement.
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="bg-gray-50 p-4 sm:p-6 flex flex-col sm:flex-row items-center justify-end gap-3">
                            ...
                            <button
                                ...
                                className="w-full sm:w-auto px-4 py-2 text-sm font-medium text-white bg-red-600 rounded-lg hover:bg-red-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed min-h-[44px] sm:min-h-0 touch-manipulation"
                            >
                                {processing
                                    ? "Removing Stock..."
                                    : "Remove Stock"}
